...
typedef struct game_context {
	...
	int npartners;						// number of active players
	int rank;							// the player order (1,2)
	bool closing_window;				// the game window was closed
	bool pending_close;					// close session pending
	int pending_results;				// a new command is issued before previous response reception
} game_context_t;
...
bool onEnd() {
	...
	if ( ctx->state >= EndingSession) {
#ifdef DEBUGC
		printf("%s for %s terminated!\n", GAME_TYPE, ctx->username);
#endif
		return true;
	}
